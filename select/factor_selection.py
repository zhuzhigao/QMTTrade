...
def check_and_download_data(stock_list):
    """强制下载所需数据"""
    # 计算开始时间 (例如过去60天)
    import datetime
...
# ================= 3. 核心计算逻辑 (已修复转置问题) =================
def calculate_factors(stock_list):
    """
    计算因子核心函数 (Updated)
    ...
    market_data = xtdata.get_market_data(
        ...
        period='1d', 
        count=MIDTERMDAYS + SHORTTERMDAYS, 
    ...
    for stock in stock_list:
        try:
            ...
            # 长度不够无法计算 MA20
            if len(closes) < MIDTERMDAYS: continue
            ...
            # 1. 动量 (Momentum): 20日涨幅
            # 逻辑: 过去20天涨了多少
            mom_short = current_price / closes[-SHORTTERMDAYS] - 1
            mom_mid = current_price / closes[-MIDTERMDAYS] - 1

            # 2. 波动率 (Volatility): 20日收益率标准差
            # 逻辑: 涨跌越平稳越好
            returns = np.diff(closes) / closes[:-1]
            vol = np.std(returns[-SHORTTERMDAYS:])

            # 3. 乖离率 (Bias): 偏离20日均线的程度
            # 逻辑: 防止追高，取绝对值看偏离度
            ma = np.mean(closes[-SHORTTERMDAYS:])
...
def get_market_sentiment():
    ...
    # 计算 20 日均线
    ma20 = index_series.rolling(20).mean().iloc[-1]
    current_price = index_series.iloc[-1]
    
    # 简单判定逻辑
    if current_price > ma20 * 1.02: return 1 # 牛市：价在均线上方
    if current_price < ma20 * 0.98: return 2 # 熊市：价在均线下方
    return 3 # 震荡
